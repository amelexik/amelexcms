...
$products_map = "<ul>";
	...
	$products_map.= "<ul>";
		foreach($categories as $key => $category){
			...
			$sub_categories = get_products_categories(array("pid"=>$category_id,"lang"=>1));
			//print_r($products);
			foreach($sub_categories as $key => $sub_category){
				
				$products_map.= "<li><a href=\"{$sub_category["url"]}\">{$sub_category["title"]}</a></li>";
				$products_map.= "<ul>";
			
				$sub_category_id = $sub_category["item_id"];
				$products = get_products(array("master_category_item_id"=>$sub_category_id,"lang"=>1));
				//print_r($products);
